...
class Evaluation():
    ...
    def queryPrecision(self, query_doc_IDs_ordered, query_id, true_doc_IDs, k):
        ...
        try:
            ...
            num_true_docs_retrieved=len(set(query_doc_IDs_ordered[:k]).intersection(set(true_doc_IDs)))
    ...
    def meanPrecision(self, doc_IDs_ordered, query_ids, qrels, k):
        ...
        precisions = []
        try:
            assert len(doc_IDs_ordered)== len(query_ids), "number of queries is not equal to number of sets of docs retrieved. \n Returning -1."
            for i in range(num_queries):
                query_doc_IDs_ordered = doc_IDs_ordered[i]
                query_id = int(query_ids[i])
                true_doc_ids = []
                for actual_dict in qrels:
                    if int(actual_dict["query_num"]) == int(query_id):
                        true_doc_ids.append(int(actual_dict['id']))
                precision = self.queryPrecision(query_doc_IDs_ordered, query_id, true_doc_ids, k)
                precisions.append(precision)
                if len(precisions)!=0:
                    return sum(precisions)/len(precisions)
                else:
                    print("Empty precision list. Returning -1.")
                    return -1
        except AssertionError as message:
            print(message)
            return -1
